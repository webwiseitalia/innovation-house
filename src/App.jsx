...
function App() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from('.hero-meta', {
        y: 30, opacity: 0, stagger: 0.15, duration: 0.7, ease: 'power3.out', delay: 1.5,
      })

      gsap.from('.hero-cta', {
        y: 40, opacity: 0, duration: 0.8, ease: 'power3.out', delay: 1.8,
      })

      gsap.from('.hero-side-img', {
        x: 120, opacity: 0, duration: 1.6, ease: 'power4.out', delay: 0.6,
      })
      ...
      if (strutturaTitle) {
        ...
        gsap.from(splitStr.words, {
          y: 100, rotateZ: () => gsap.utils.random(-3, 3), opacity: 0,
          stagger: { each: 0.05 }, duration: 1.2, ease: 'power4.out',
          scrollTrigger: { trigger: strutturaTitle, start: 'top 80%' }
        })
      }

      gsap.from('.str-text-block', {
        x: -80, opacity: 0, duration: 1.3, ease: 'expo.out',
      ...
      gsap.from('.str-img-float', {
        y: 100, rotation: -6, opacity: 0, duration: 1.4, ease: 'power4.out',
      ...
      if (roomsTitle) {
        ...
        gsap.from(splitRooms.chars, {
          y: -80, opacity: 0, stagger: { each: 0.025, from: 'center' },
      ...
      if (srvItems.length) {
        srvItems.forEach((item, i) => {
          gsap.from(item, {
            y: gsap.utils.random(20, 60), opacity: 0, duration: 0.7,
            ease: 'power3.out', delay: i * 0.06,
      ...
      if (terTitle) {
        ...
        gsap.from(splitTer.words, {
          y: 60, opacity: 0, stagger: 0.07, duration: 1, ease: 'power4.out',
      ...
      terCards.forEach((card, i) => {
        const dir = i % 2 === 0 ? -1 : 1
        gsap.from(card, {
          x: dir * 140, y: 50, rotation: dir * 3, opacity: 0,
          duration: 1.3, ease: 'power4.out',
          scrollTrigger: { trigger: card, start: 'top 88%' }
        })
      })

      // === GALLERIA HORIZONTAL SCROLL ===
      ...
      // === POSIZIONE ===
      gsap.from('.pos-map', {
        scale: 0.85, opacity: 0, duration: 1.4, ease: 'power3.out',
      ...
      gsap.from('.contact-side', {
        x: 120, rotation: 2, opacity: 0, duration: 1.4, ease: 'power4.out',
  ...
  return (
    <div className="noise-bg relative">
      ...
      <AnimatePresence>
        {lightbox.open && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-[#1a3a4a]/95 flex items-center justify-center p-4 cursor-pointer"
          ...
          >
            ...
            />
            <span className="absolute top-10 right-10 font-heading text-xs tracking-[0.5em] uppercase text-white/80 hover:text-white transition-colors cursor-pointer">Chiudi</span>
          </motion.div>
        )}
      </AnimatePresence>

      {/* ═══ NAV ═══ */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${scrolled ? 'bg-[#e8e2db]/90 backdrop-blur-2xl shadow-sm' : ''}`}>
        <div className="flex items-center justify-between h-14 md:h-16 px-6 md:px-10 lg:px-16">
          <button onClick={() => scrollTo('hero')} className={`font-heading text-xs md:text-sm tracking-[0.5em] uppercase transition-colors ${scrolled ? 'text-[#2d2a26]' : 'text-white'} hover:text-[#5bb8c1]`}>
            Innovation<br className="md:hidden" /> House
          </button>

          <div className="hidden lg:flex items-center gap-12">
            {navLinks.map((l) => (
              <button key={l.id} onClick={() => scrollTo(l.id)} className={`font-body text-xs tracking-[0.2em] uppercase transition-colors duration-500 ${scrolled ? 'text-[#2d2a26]/90 hover:text-[#5bb8c1]' : 'text-white/90 hover:text-[#5bb8c1]'}`}>
                {l.label}
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center gap-6">
            <button onClick={() => scrollTo('contatti')} className={`hidden md:block font-heading text-xs tracking-[0.35em] uppercase px-7 py-3 transition-all duration-500 ${scrolled ? 'text-[#5bb8c1] border border-[#5bb8c1]/40 hover:bg-[#5bb8c1] hover:text-white' : 'text-white border border-white/30 hover:bg-white hover:text-[#2d2a26]'}`}>
              Prenota
            </button>
            <button onClick={() => setMenuOpen(!menuOpen)} className="lg:hidden w-10 h-10 flex flex-col items-end justify-center gap-[5px]" aria-label="Menu">
              <span className={`block h-[1px] transition-all duration-500 ${scrolled ? 'bg-[#2d2a26]' : 'bg-white'} ${menuOpen ? 'w-7 rotate-45 translate-y-[3px]' : 'w-7'}`} />
              <span className={`block h-[1px] transition-all duration-500 ${scrolled ? 'bg-[#2d2a26]' : 'bg-white'} ${menuOpen ? 'w-7 -rotate-45 -translate-y-[3px]' : 'w-4'}`} />
            </button>
          </div>
        </div>
        ...
        <AnimatePresence>
          {menuOpen && (
            <motion.div
              ...
              className="lg:hidden fixed inset-0 top-20 bg-[#e8e2db] z-40 flex flex-col justify-center px-10"
            >
              {navLinks.map((l, i) => (
                <motion.button
                  key={l.id}
                  ...
                  className="block text-left font-heading text-[clamp(2rem,8vw,4rem)] font-normal text-[#2d2a26] hover:text-[#5bb8c1] hover:pl-4 transition-all duration-500 py-3"
                >
                  {l.label}
                </motion.button>
              ))}
        ...
        <div className="relative z-10 min-h-screen flex flex-col justify-end pb-16 md:pb-24 pt-32">
          <div className="px-6 md:px-10 lg:px-16">
            <div className="max-w-[920px]">
              <p className="hero-meta font-body text-xs md:text-sm tracking-[0.6em] uppercase text-[#8dd8e0] mb-6 md:mb-10">
              ...
              <p className="hero-sub font-body text-white/90 text-lg md:text-xl leading-[1.8] max-w-md md:ml-[8vw] mb-12">
              ...
              <div className="flex flex-wrap items-end gap-6 md:gap-12">
                <button onClick={() => scrollTo('contatti')} className="hero-cta group font-heading text-xs md:text-sm tracking-[0.4em] uppercase bg-[#5bb8c1] text-white px-8 md:px-10 py-4 md:py-5 hover:bg-white hover:text-[#2d2a26] transition-colors duration-500 flex items-center gap-3">
                  Prenota
                ...
                <div className="hero-meta flex items-center gap-4">
                  ...
                  <span className="font-heading text-[40px] md:text-[56px] font-bold text-white/25 leading-none">9.1</span>
                  <div>
                    <span className="font-body text-xs tracking-[0.2em] uppercase text-white/90 block">Superb</span>
                    <span className="font-body text-[11px] text-white/70">bedandbreakfast.eu</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* ═══════════════════════ MARQUEE ═══════════════════════ */}
      <div className="py-6 md:py-8 border-y border-[#5bb8c1]/15 overflow-hidden bg-[#d0e7e9]">
            ...
            <div key={rep} className="flex items-center gap-8 md:gap-12">
              {['Vista Mare', 'Piscina', 'Cinque Terre · 8 min', 'Parcheggio Gratuito', 'Golfo dei Poeti', '9.1/10 Superb', 'Pet Friendly'].map((t, i) => (
                <span key={`${rep}-${i}`} className="flex items-center gap-4 md:gap-6">
                  <span className="font-heading text-xs md:text-sm tracking-[0.3em] uppercase text-[#2d2a26]/90">{t}</span>
                  <span className="w-1.5 h-1.5 bg-[#5bb8c1]/60 rotate-45" />
                </span>
              ))}
            </div>
          ))}
      ...
      {/* ═══════════════════════ LA STRUTTURA ═══════════════════════ */}
      <section id="struttura" className="texture-crosshatch relative py-32 md:py-48 overflow-hidden bg-[#e8e2db]">
        <div className="px-6 md:px-10 lg:px-16">
          <div className="mb-20 md:mb-36 w-full">
            <span className="font-body text-xs tracking-[0.6em] uppercase text-[#5bb8c1] block mb-5">( 01 )</span>
            <h2 className="struttura-title font-heading text-[clamp(2.8rem,7vw,6.5rem)] font-bold text-[#2d2a26] leading-[0.92] tracking-[-0.02em]">
          ...
          <div className="relative">
            <div className="str-text-block md:w-[42%] lg:w-[35%]">
              <p className="font-body text-[#2d2a26] text-base md:text-lg leading-[2] mb-8">
              ...
              <p className="font-body text-[#2d2a26]/90 text-base leading-[2] mb-16">
              ...
              <div className="space-y-6">
                {['Posizione panoramica con vista mare', 'Atmosfera rilassante', 'Base ideale per escursioni', 'Parcheggio privato gratuito'].map((t, i) => (
                  <div key={t} className="flex items-start gap-5 group">
                    <span className="font-heading text-xs text-[#5bb8c1] mt-1">0{i + 1}</span>
                    <span className="font-body text-[#2d2a26] text-base group-hover:text-[#5bb8c1] transition-colors duration-500">{t}</span>
                  </div>
                ))}
              </div>

              <div className="mt-24 md:mt-32 border-l-2 border-[#5bb8c1]/40 pl-8">
                <span className="font-heading text-[4.5rem] font-bold text-[#5bb8c1]/20 leading-none block">9.1</span>
                <span className="font-heading text-[#2d2a26] text-lg font-medium tracking-wide block mt-2">&ldquo;Superb&rdquo;</span>
                <span className="font-body text-[#2d2a26]/80 text-xs tracking-[0.3em] uppercase block mt-3">Bedandbreakfast.eu · 10 recensioni</span>
              </div>
            </div>
      ...
      {/* ═══════════════════════ LE CAMERE ═══════════════════════ */}
      <section id="camere" className="texture-waves py-32 md:py-48 bg-[#c8e3e5] relative">
        <div className="absolute top-16 right-6 md:right-16 font-heading text-[20vw] font-bold text-[#5bb8c1]/[0.06] leading-none select-none pointer-events-none">
          02
        </div>

        <div className="px-6 md:px-10 lg:px-16 w-full relative z-10">
          <div className="md:text-right mb-16 md:mb-28">
            <span className="font-body text-xs tracking-[0.6em] uppercase text-[#5bb8c1]">( 02 )</span>
            <h2 className="rooms-title font-heading text-[clamp(2.5rem,6vw,5.5rem)] font-bold text-[#2d2a26] leading-[0.95] mt-5 tracking-[-0.02em]">
              Le Camere
            </h2>
            <p className="font-body text-[#2d2a26]/90 text-base mt-6 md:ml-auto max-w-md leading-[1.9]">
              Bagno privato, aria condizionata, TV e WiFi gratuito in ogni camera
            </p>
          </div>

          <div className="space-y-0 border-t border-[#2d2a26]/10">
            {rooms.map((room, idx) => (
              <div key={room.name} className="border-b border-[#2d2a26]/10">
                <button
                ...
                >
                  <div className="flex items-baseline gap-4 md:gap-8">
                    <span className="font-heading text-xs text-[#5bb8c1]">{room.num}</span>
                    <span className={`font-heading text-xl md:text-3xl font-normal tracking-wide transition-colors duration-500 ${expandedRoom === idx ? 'text-[#5bb8c1]' : 'text-[#2d2a26] group-hover:text-[#5bb8c1]'}`}>
                      {room.name}
                    </span>
                  </div>
                  <div className="flex items-center gap-6">
                    <span className="hidden md:block font-body text-xs tracking-[0.2em] uppercase text-[#2d2a26]/80">{room.tagline}</span>
                    {expandedRoom === idx ? (
                      <Minus className="w-4 h-4 text-[#5bb8c1]" strokeWidth={1} />
                    ) : (
                      <Plus className="w-4 h-4 text-[#2d2a26]/30 group-hover:text-[#5bb8c1] transition-colors" strokeWidth={1} />
                    )}
                  </div>
                </button>
                      ...
                      <div className="pb-12 md:pb-16">
                        <div className="grid grid-cols-12 gap-4 md:gap-6">
                          ...
                          <div className="col-span-12 md:col-span-5 md:col-start-8 md:-mt-[180px] relative z-10">
                            <div className="bg-white/90 backdrop-blur-sm border border-[#5bb8c1]/15 p-8 md:p-10 rounded-sm shadow-lg shadow-[#5bb8c1]/8">
                              <p className="font-body text-[#2d2a26] text-base leading-[1.9] mb-8">{room.description}</p>
                              <div className="grid grid-cols-2 gap-x-6 gap-y-3 mb-8">
                                {room.features.map((f) => (
                                  <div key={f} className="flex items-center gap-3">
                                    <span className="w-3 h-[1px] bg-[#5bb8c1]" />
                                    <span className="font-body text-[#2d2a26]/90 text-sm">{f}</span>
                                  </div>
                                ))}
                              </div>
                              <button onClick={() => scrollTo('contatti')} className="font-heading text-xs tracking-[0.35em] uppercase text-white bg-[#5bb8c1] px-8 py-4 hover:bg-[#4a9da5] transition-all duration-500 flex items-center gap-3 group">
          ...
          <div className="mt-20 md:mt-28 flex flex-wrap gap-x-6 md:gap-x-10 gap-y-2">
            {['Lenzuola', 'Asciugamani', 'Asciugacapelli', 'Set cortesia', 'Armadio', 'TV', 'A/C', 'WiFi'].map((item) => (
              <span key={item} className="font-body text-[#2d2a26]/80 text-xs tracking-[0.2em] uppercase">{item}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      {/* ═══════════════════════ SERVIZI ═══════════════════════ */}
      <section id="servizi" className="texture-dots py-32 md:py-48 px-6 md:px-10 lg:px-16 relative overflow-hidden bg-[#e8e2db]">
        <div className="absolute top-[20%] -right-[10%] font-heading text-[18vw] font-bold text-[#5bb8c1]/[0.05] leading-none select-none pointer-events-none -rotate-12">
          COMFORT
        </div>
        ...
        <div className="w-full relative z-10">
          <div className="mb-20 md:mb-32 flex flex-col md:flex-row md:items-end md:justify-between gap-8">
            <div>
              <span className="font-body text-xs tracking-[0.6em] uppercase text-[#5bb8c1] block mb-5">( 03 )</span>
              <h2 className="font-heading text-[clamp(2.5rem,5.5vw,5rem)] font-bold text-[#2d2a26] leading-[0.95] tracking-[-0.02em]">
                Tutto per il<br />vostro relax
              </h2>
            </div>
            <p className="font-body text-[#2d2a26]/90 text-base max-w-xs leading-[2] md:pb-2">
              Ogni servizio incluso. Nessun costo nascosto.
            </p>
          </div>

          <div className="srv-grid grid grid-cols-2 md:grid-cols-4 gap-[1px] bg-[#5bb8c1]/15">
            {services.map((s) => {
              const Icon = s.icon
              return (
                <div key={s.title} className="srv-item bg-[#e8e2db] p-7 md:p-10 group hover:bg-[#d0e7e9] transition-all duration-700">
                  <Icon className="w-5 h-5 md:w-6 md:h-6 text-[#5bb8c1] group-hover:text-[#4a9da5] mb-5 md:mb-8 transition-colors duration-500" strokeWidth={1.2} />
                  <h3 className="font-heading text-base md:text-lg text-[#2d2a26] font-medium mb-1 transition-colors duration-500">{s.title}</h3>
                  <p className="font-body text-sm text-[#2d2a26]/85 group-hover:text-[#2d2a26] transition-colors duration-500">{s.desc}</p>
                </div>
              )
            })}
          </div>
          ...
          <div className="mt-14 flex items-center gap-5">
            <div className="w-16 h-[1px] bg-[#5bb8c1]/30" />
            <p className="font-body text-sm text-[#2d2a26]/85 tracking-[0.15em]">Lingue parlate: Italiano · English · Español</p>
          </div>
        </div>
      </section>

      {/* ═══════════════════════ GALLERIA (Horizontal Scroll) ═══════════════════════ */}
      <div ref={gallerySectionRef} className="relative" style={{ height: '300vh' }}>
        <div className="sticky top-0 h-screen bg-[#1a3a4a] overflow-hidden flex flex-col justify-center">
          <div className="px-6 md:px-10 lg:px-16 mb-8 md:mb-10">
            <span className="font-body text-xs tracking-[0.6em] uppercase text-[#5bb8c1] block mb-4">Galleria</span>
            <h2 className="font-heading text-[clamp(1.8rem,4vw,3rem)] font-bold text-white leading-[0.95] tracking-[-0.02em]">I nostri spazi</h2>
          </div>
          <div ref={galleryTrackRef} className="flex gap-4 md:gap-5 px-6 items-center will-change-transform">
            {[
      ...
      {/* ═══════════════════════ POSIZIONE ═══════════════════════ */}
      <section id="posizione" className="texture-stripes py-32 md:py-48 px-6 md:px-10 lg:px-16 bg-[#e8e2db]">
        <div className="w-full">
          <div className="mb-16 md:mb-24">
            <span className="font-body text-xs tracking-[0.6em] uppercase text-[#5bb8c1] block mb-5">( 04 )</span>
            <h2 className="font-heading text-[clamp(2rem,4.5vw,4rem)] font-bold text-[#2d2a26] leading-[0.95]">
              Dove siamo
            </h2>
          </div>
          ...
          <div className="grid grid-cols-12 gap-6 md:gap-10">
            <div className="pos-map col-span-12 md:col-span-7 -ml-6 md:-ml-10 lg:-ml-16">
              <div className="h-[350px] md:h-[550px] rounded-sm overflow-hidden">
                <iframe
            ...
            <div className="pos-info col-span-12 md:col-span-5 md:pt-24">
              <div className="space-y-0">
                {[
                  ...
                  <div key={item.place} className="flex items-baseline justify-between py-5 border-b border-[#2d2a26]/8 group hover:border-[#5bb8c1]/30 transition-colors">
                    <div>
                      <span className="font-heading text-base font-medium text-[#2d2a26] group-hover:text-[#5bb8c1] transition-colors block">{item.place}</span>
                      <span className="font-body text-xs text-[#2d2a26]/80 block mt-1">{item.note}</span>
                    </div>
                    <span className="font-heading text-base font-bold text-[#5bb8c1]">{item.dist}</span>
                  </div>
                ))}
              </div>

              <div className="mt-14 border border-[#5bb8c1]/20 p-8 bg-[#d0e7e9]/50 rounded-sm">
                <h4 className="font-heading text-sm font-medium text-[#2d2a26] mb-5 tracking-wide">Come raggiungerci</h4>
                <div className="space-y-3 font-body text-base text-[#2d2a26]/90 leading-[1.9]">
                  <p><span className="text-[#2d2a26] font-medium">Auto:</span> Uscita A12 La Spezia — seguire per Pianello</p>
                  <p><span className="text-[#2d2a26] font-medium">Treno:</span> La Spezia Centrale, poi taxi (5 km)</p>
                  <p><span className="text-[#2d2a26] font-medium">Aereo:</span> Pisa (87 km) o Genova (100 km)</p>
                </div>
                <div className="mt-6 flex items-center gap-3">
                  <CircleParking className="w-4 h-4 text-[#5bb8c1]" strokeWidth={1.2} />
                  <span className="font-body text-sm text-[#5bb8c1] tracking-wide font-medium">Parcheggio gratuito</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* ═══════════════════════ TERRITORIO ═══════════════════════ */}
      <section id="territorio" className="texture-diamonds py-32 md:py-48 relative overflow-hidden bg-[#c8e3e5]">
        <div className="px-6 md:px-10 lg:px-16 w-full relative z-10">
          <div className="mb-20 md:mb-36">
            <span className="font-body text-xs tracking-[0.6em] uppercase text-[#5bb8c1] block mb-5">( 05 )</span>
            <h2 className="ter-title font-heading text-[clamp(2.5rem,7vw,6.5rem)] font-bold text-[#2d2a26] leading-[0.92] tracking-[-0.02em] max-w-4xl">
              Il Golfo<br />dei Poeti
            </h2>
            <p className="font-body text-[#2d2a26]/90 text-lg max-w-lg leading-[2] mt-8 md:ml-[12vw]">
          ...
          <div className="grid grid-cols-12 gap-5 md:gap-6">
            <div className="ter-card col-span-12 md:col-span-7 border border-[#5bb8c1]/15 hover:border-[#5bb8c1]/30 transition-all duration-700 bg-white/80 backdrop-blur-sm overflow-hidden rounded-sm shadow-sm">
              ...
              <div className="p-8 md:p-14">
                ...
                <h3 className="font-heading text-2xl md:text-4xl text-[#2d2a26] font-normal mt-6 mb-2 tracking-wide">Cinque Terre</h3>
                <span className="font-body text-xs tracking-[0.4em] uppercase text-[#c8956c]">Patrimonio UNESCO</span>
                <p className="font-body text-[#2d2a26]/90 text-base leading-[2] mt-6 max-w-lg">
            ...
            <div className="ter-card col-span-12 md:col-span-5 md:mt-20 border border-[#5bb8c1]/15 hover:border-[#5bb8c1]/30 transition-all duration-700 bg-white/80 backdrop-blur-sm overflow-hidden rounded-sm shadow-sm">
              ...
              <div className="p-8 md:p-12">
                ...
                <h3 className="font-heading text-xl md:text-2xl text-[#2d2a26] font-normal mt-6 mb-2">Portovenere</h3>
                <span className="font-body text-xs tracking-[0.4em] uppercase text-[#c8956c]">Patrimonio UNESCO</span>
                <p className="font-body text-[#2d2a26]/90 text-base leading-[2] mt-6">
            ...
            <div className="ter-card col-span-12 md:col-span-5 md:-mt-12 border border-[#5bb8c1]/15 hover:border-[#5bb8c1]/30 transition-all duration-700 bg-white/80 backdrop-blur-sm overflow-hidden rounded-sm shadow-sm">
              ...
              <div className="p-8 md:p-12">
                ...
                <h3 className="font-heading text-xl md:text-2xl text-[#2d2a26] font-normal mt-6 mb-2">Lerici e Tellaro</h3>
                <span className="font-body text-xs tracking-[0.4em] uppercase text-[#c8956c]">Borghi pittoreschi</span>
                <p className="font-body text-[#2d2a26]/90 text-base leading-[2] mt-6">
            ...
            <div className="ter-card col-span-12 md:col-span-7 border border-[#5bb8c1]/15 hover:border-[#5bb8c1]/30 transition-all duration-700 bg-white/80 backdrop-blur-sm overflow-hidden rounded-sm shadow-sm">
              ...
              <div className="p-8 md:p-14">
                ...
                <h3 className="font-heading text-2xl md:text-3xl text-[#2d2a26] font-normal mt-6 mb-2 tracking-wide">Escursioni</h3>
                <span className="font-body text-xs tracking-[0.4em] uppercase text-[#c8956c]">Trekking e natura</span>
                <p className="font-body text-[#2d2a26]/90 text-base leading-[2] mt-6 max-w-lg">
          ...
          <div className="mt-24 md:mt-32 border-t border-[#2d2a26]/8 pt-10">
            <h3 className="font-heading text-lg text-[#2d2a26] font-medium mb-6 tracking-wide">La Spezia</h3>
            <div className="flex flex-wrap gap-x-6 md:gap-x-10 gap-y-3">
              {['Museo Amedeo Lia', 'Museo Tecnico Navale', 'Castello San Giorgio', 'CAMEC', 'Museo del Sigillo', 'Passeggiata Morin', 'Via del Prione', 'Arsenale Militare'].map((p) => (
                <span key={p} className="font-body text-[#2d2a26]/85 text-sm tracking-[0.1em] hover:text-[#5bb8c1] transition-colors duration-500 cursor-default">{p}</span>
              ))}
            </div>
          </div>
        </div>
      ...
      {/* ═══════════════════════ CONTATTI ═══════════════════════ */}
      <section id="contatti" className="texture-circles py-32 md:py-48 px-6 md:px-10 lg:px-16 bg-[#e8e2db]">
        <div className="w-full">
          <div className="mb-16 md:mb-28">
            <span className="font-body text-xs tracking-[0.6em] uppercase text-[#5bb8c1] block mb-5">( 06 )</span>
            <h2 className="font-heading text-[clamp(2.5rem,6vw,5.5rem)] font-bold text-[#2d2a26] leading-[0.95] tracking-[-0.02em] max-w-3xl">
          ...
          <div className="grid grid-cols-12 gap-8 md:gap-16">
            <div className="contact-form-wrap col-span-12 lg:col-span-7">
              <form onSubmit={handleSubmit} className="space-y-7">
                <div className="grid md:grid-cols-2 gap-7">
                  <div>
                    <label className="block font-heading text-xs tracking-[0.2em] uppercase text-[#2d2a26] mb-3">Nome *</label>
                    <input type="text" required value={formData.nome} onChange={(e) => setFormData({ ...formData, nome: e.target.value })} className="w-full px-0 py-4 bg-transparent border-0 border-b border-[#2d2a26]/15 focus:border-[#5bb8c1] outline-none transition-colors duration-500 font-body text-[#2d2a26] placeholder:text-[#2d2a26]/40 text-base" placeholder="Il tuo nome" />
                  </div>
                  <div>
                    <label className="block font-heading text-xs tracking-[0.2em] uppercase text-[#2d2a26] mb-3">Email *</label>
                    <input type="email" required value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="w-full px-0 py-4 bg-transparent border-0 border-b border-[#2d2a26]/15 focus:border-[#5bb8c1] outline-none transition-colors duration-500 font-body text-[#2d2a26] placeholder:text-[#2d2a26]/40 text-base" placeholder="la-tua@email.com" />
                  </div>
                </div>

                <div>
                  <label className="block font-heading text-xs tracking-[0.2em] uppercase text-[#2d2a26] mb-3">Telefono</label>
                  <input type="tel" value={formData.telefono} onChange={(e) => setFormData({ ...formData, telefono: e.target.value })} className="w-full px-0 py-4 bg-transparent border-0 border-b border-[#2d2a26]/15 focus:border-[#5bb8c1] outline-none transition-colors duration-500 font-body text-[#2d2a26] placeholder:text-[#2d2a26]/40 text-base" placeholder="+39 ..." />
                </div>

                <div className="grid grid-cols-3 gap-7">
                  <div>
                    <label className="block font-heading text-xs tracking-[0.2em] uppercase text-[#2d2a26] mb-3">Check-in *</label>
                    <input type="date" required value={formData.checkin} onChange={(e) => setFormData({ ...formData, checkin: e.target.value })} className="w-full px-0 py-4 bg-transparent border-0 border-b border-[#2d2a26]/15 focus:border-[#5bb8c1] outline-none transition-colors duration-500 font-body text-[#2d2a26] text-base" />
                  </div>
                  <div>
                    <label className="block font-heading text-xs tracking-[0.2em] uppercase text-[#2d2a26] mb-3">Check-out *</label>
                    <input type="date" required value={formData.checkout} onChange={(e) => setFormData({ ...formData, checkout: e.target.value })} className="w-full px-0 py-4 bg-transparent border-0 border-b border-[#2d2a26]/15 focus:border-[#5bb8c1] outline-none transition-colors duration-500 font-body text-[#2d2a26] text-base" />
                  </div>
                  <div>
                    <label className="block font-heading text-xs tracking-[0.2em] uppercase text-[#2d2a26] mb-3">Ospiti</label>
                    <select value={formData.ospiti} onChange={(e) => setFormData({ ...formData, ospiti: e.target.value })} className="w-full px-0 py-4 bg-transparent border-0 border-b border-[#2d2a26]/15 focus:border-[#5bb8c1] outline-none transition-colors duration-500 font-body text-[#2d2a26] text-base">
                ...
                <div>
                  <label className="block font-heading text-xs tracking-[0.2em] uppercase text-[#2d2a26] mb-3">Camera</label>
                  <select value={formData.camera} onChange={(e) => setFormData({ ...formData, camera: e.target.value })} className="w-full px-0 py-4 bg-transparent border-0 border-b border-[#2d2a26]/15 focus:border-[#5bb8c1] outline-none transition-colors duration-500 font-body text-[#2d2a26] text-base">
                    <option value="">Seleziona</option>
                    <option value="Deluxe Room">Deluxe Room — Vista Mare + Terrazza</option>
                    <option value="Queen Room">Queen Room — Vista Mare</option>
                    <option value="Superior Queen Room">Superior Queen Room — Vista Piscina</option>
                  </select>
                </div>

                <div>
                  <label className="block font-heading text-xs tracking-[0.2em] uppercase text-[#2d2a26] mb-3">Richieste</label>
                  <textarea rows={3} value={formData.messaggio} onChange={(e) => setFormData({ ...formData, messaggio: e.target.value })} className="w-full px-0 py-4 bg-transparent border-0 border-b border-[#2d2a26]/15 focus:border-[#5bb8c1] outline-none transition-colors duration-500 font-body text-[#2d2a26] placeholder:text-[#2d2a26]/40 resize-none text-sm" placeholder="Animali, orario di arrivo, richieste particolari..." />
                </div>
                ...
                <button type="submit" className="font-heading text-xs tracking-[0.4em] uppercase px-12 py-5 bg-[#5bb8c1] text-white hover:bg-[#4a9da5] transition-colors duration-500 flex items-center gap-3 group w-full md:w-auto justify-center rounded-sm">
                  Invia Richiesta
                  <ArrowUpRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
                </button>
              </form>
            ...
            <div className="contact-side col-span-12 lg:col-span-4 lg:col-start-9 space-y-10 lg:pt-12">
              ...
              <div>
                <h3 className="font-heading text-xs tracking-[0.35em] uppercase text-[#5bb8c1] mb-6">Politiche</h3>
                <div className="space-y-4">
                  {[
                    ...
                    <div key={p.l} className="flex justify-between border-b border-[#2d2a26]/8 pb-3">
                      <span className="font-body text-sm text-[#2d2a26]/85">{p.l}</span>
                      <span className="font-body text-sm text-[#2d2a26] font-medium">{p.v}</span>
                    </div>
                  ))}
                </div>
              </div>

              <div className="border border-[#5bb8c1]/25 p-8 bg-[#d0e7e9]/60 rounded-sm">
                <p className="font-heading text-[#2d2a26] text-base font-medium tracking-wide leading-relaxed">
                  Prenotazione diretta<br />
                  <span className="text-[#5bb8c1] font-medium">= Miglior tariffa garantita</span>
                </p>
              </div>
            </div>
          </div>
      ...
      {/* ═══════════════════════ FOOTER ═══════════════════════ */}
      <footer className="texture-lines bg-[#1a3a4a] text-white py-20 md:py-28 px-6 md:px-10 lg:px-16">
        <div className="w-full">
          <div className="mb-16 md:mb-24 flex flex-col md:flex-row md:items-end md:justify-between gap-8">
            <h3 className="font-heading text-[clamp(2rem,5vw,4rem)] font-bold text-white leading-[0.95]">
              Innovation<br />House
            </h3>
            <div className="flex gap-6">
              <a href="#" aria-label="Facebook" className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/80 hover:text-[#5bb8c1] hover:border-[#5bb8c1]/40 transition-all duration-500">
                <Facebook className="w-4 h-4" strokeWidth={1.2} />
              </a>
              <a href="#" aria-label="Instagram" className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/80 hover:text-[#5bb8c1] hover:border-[#5bb8c1]/40 transition-all duration-500">
          ...
          <div className="grid grid-cols-12 gap-8">
            <div className="col-span-12 md:col-span-4">
              <p className="font-body text-white/85 text-base leading-[2] max-w-xs">
            ...
            <div className="col-span-6 md:col-span-3 md:col-start-6">
              <div className="space-y-3">
                {navLinks.map((link) => (
                  <button key={link.id} onClick={() => scrollTo(link.id)} className="block font-body text-white/85 hover:text-[#5bb8c1] text-base transition-colors">
                    {link.label}
                  </button>
                ))}
              </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <p className="font-body text-white/70 text-xs tracking-[0.15em]">
